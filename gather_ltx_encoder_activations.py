...
def gather_vae_latents(config: VaeLatentConfig):
    ...
    dummy = torch.randn(1, 3, dummy_frames, 128, 128,
                        device=config.device).clamp_(-1, 1)
    ...
    if config.is_image_mode:
        ...
        def frame_reader(path, _): return read_all_frames(path)
    else:
        frame_reader = read_consecutive_frames_random_start

    prefetcher = FramePrefetcher(
        remaining, config.num_frames, config.prefetch_workers, frame_reader)
    ...
    logger.info(
        f"Done. {writer.total_tokens} tokens, {writer.shard_index} shards. Output: {config.output_dir}")
